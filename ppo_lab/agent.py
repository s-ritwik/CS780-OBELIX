...
class FeatureTracker:
    ...
    def observe_transition(self, action_idx: int, obs: np.ndarray) -> None:
        ...
        self.blind_turn_steps = (
            min(self.config.turn_clip, self.blind_turn_steps + 1.0)
            if blind and turned
            else 0.0
        )
        if obs_arr[17] > 0.5:
...
def _checkpoint_path() -> str:
    ...
    if env_override:
        return env_override if os.path.isabs(env_override) else os.path.join(here, env_override)
