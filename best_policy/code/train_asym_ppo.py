...
EXPERTS_DIR = os.path.join(CODE_DIR, "experts")
...
def resolve_expert_path(expert: str, repo_dir: str) -> str:
    experts_dir = os.path.join(repo_dir, "experts")
    mapping = {
        "wall_submission": os.path.join(experts_dir, "agent_wall_submission.py"),
        "nowall": os.path.join(experts_dir, "agent_nowall.py"),
        "eval300": os.path.join(experts_dir, "agent_eval300.py"),
        "handmade": os.path.join(experts_dir, "handmade_agent.py"),
    }
    if expert in mapping:
        return mapping[expert]
    if os.path.isfile(expert):
        return os.path.abspath(expert)
    candidate = os.path.join(experts_dir, expert)
