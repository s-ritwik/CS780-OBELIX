...
def _reset(obs_arr: np.ndarray, rng: np.random.Generator) -> None:
    global _LAST_RNG_ID, _STEP, _MODE, _FIRST_OBS, _STATS, _PROBE_REWARD_PROXY, _SPIN_COUNT, _SPIN_SEEN, _SPIN_LAST_SUM, _RECOVERY_PLAN
    _LAST_RNG_ID = id(rng)
    _STEP = 0
    _MODE = None
...
def policy(obs: np.ndarray, rng: np.random.Generator) -> str:
    global _STEP, _MODE, _STATS, _PROBE_REWARD_PROXY, _SPIN_COUNT, _SPIN_SEEN, _SPIN_LAST_SUM, _RECOVERY_PLAN
    _load_once()
    obs_arr = np.asarray(obs, dtype=np.float32)
    ...
    if _RECOVERY_PLAN:
        _STEP += 1
        return _RECOVERY_PLAN.pop(0)
    if obs_arr[17] > 0.5:
        left = float(np.sum(obs_arr[:4]))
        right = float(np.sum(obs_arr[12:16]))
        turn = 'R45' if left > right else 'L45'
        _RECOVERY_PLAN = [turn, turn, turn, turn, 'FW']
        _STEP += 1
        return _RECOVERY_PLAN.pop(0)
