...
def _load_once() -> None:
    ...
    _ROUTER.eval()
    _ROUTER_THRESHOLD = max(float(router_ckpt.get('threshold', 0.9)), 0.835)
...
def policy(obs: np.ndarray, rng: np.random.Generator) -> str:
    ...
    current_sum = float(np.sum(obs_arr[:16]))
    risky_side_start = first_sum >= 1.0 and first_front <= 0.0 and wall_prob >= 0.70
    late_empty_wall = (
        first_sum <= 0.0
        and wall_prob >= 0.78
        and float(_STATS[11]) >= 20.0
        and current_sum <= 0.0
    )
    late_front_wall = (
        first_sum <= 0.0
    ...
    if wall_prob >= _ROUTER_THRESHOLD or risky_side_start or late_empty_wall or late_front_wall:
        _MODE = 'wall'
        ...
        _MODE = 'nowall'
        action = _NOWALL.act(obs_arr, rng)
    _STEP += 1
    return action
