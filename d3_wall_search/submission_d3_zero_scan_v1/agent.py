...
def _reset(obs_arr: np.ndarray, rng: np.random.Generator) -> None:
    ...
    global _ZERO_SCAN, _ZERO_SCAN_STEP, _ZERO_SCAN_SEEN, _ZERO_SCAN_FIRST_SEEN
    _LAST_RNG_ID = id(rng)
    _STEP = 0
    _MODE = None
    ...
    _SPIN_SEEN = 0
    _SPIN_LAST_SUM = 0.0
    _ZERO_SCAN = bool(np.sum(obs_arr[:16]) <= 0.0)
    _ZERO_SCAN_STEP = 0
    _ZERO_SCAN_SEEN = 0
    _ZERO_SCAN_FIRST_SEEN = 0
...
def policy(obs: np.ndarray, rng: np.random.Generator) -> str:
    ...
    if _ZERO_SCAN and _MODE is None:
        visible = int(np.sum(obs_arr[:16]) > 0.0)
        if _ZERO_SCAN_STEP > 0 and _ZERO_SCAN_STEP <= 8 and visible:
            _ZERO_SCAN_SEEN += 1
            if _ZERO_SCAN_FIRST_SEEN == 0:
                _ZERO_SCAN_FIRST_SEEN = _ZERO_SCAN_STEP
        if _ZERO_SCAN_STEP < 8:
            _ZERO_SCAN_STEP += 1
            _STEP += 1
            return 'L45'
        if _ZERO_SCAN_SEEN == 0 and _ZERO_SCAN_STEP == 8:
            _ZERO_SCAN_STEP += 1
            _STEP += 1
            return 'R45'
        if _ZERO_SCAN_SEEN == 0 and _ZERO_SCAN_STEP == 9:
            _ZERO_SCAN_STEP += 1
            _STEP += 1
            return 'FW'
        if _ZERO_SCAN_SEEN == 0:
            _MODE = 'wall' if float(np.sum(obs_arr[:16])) > 0.0 else 'nowall'
        elif _ZERO_SCAN_FIRST_SEEN == 2:
            _MODE = 'nowall'
        else:
            _MODE = 'wall'
        _ZERO_SCAN = False
