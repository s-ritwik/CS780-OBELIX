...
_DETECTOR: Optional[WallDetectorGRU] = None
_ROUTER_THRESHOLD = 0.9
_DETECTOR_THRESHOLD = 0.5
_DETECTOR_PROBE_STEPS = 80
_PROBE_STEPS = 20
...
_SPIN_SEEN = 0
_SPIN_LAST_SUM = 0.0
_DETECTOR_TRACE: list[np.ndarray] = []
_DETECTOR_LAST_ACTION = -1
...
def _load_once() -> None:
    global _WALL, _NOWALL, _NOWALL_V1, _ROUTER, _DETECTOR, _ROUTER_THRESHOLD, _DETECTOR_THRESHOLD, _DETECTOR_PROBE_STEPS, _PROBE_STEPS
    if (
        _WALL is not None
        and _NOWALL is not None
        and _NOWALL_V1 is not None
    ...
    detector_ckpt = bundle['detector']
    _DETECTOR = WallDetectorGRU(
        input_dim=int(detector_ckpt['input_dim']),
        hidden_dim=int(detector_ckpt['hidden_dim']),
        layers=int(detector_ckpt['layers']),
        dropout=float(detector_ckpt.get('dropout', 0.0)),
    )
    _DETECTOR.load_state_dict(detector_ckpt['state_dict'], strict=True)
    _DETECTOR.eval()
    _DETECTOR_THRESHOLD = float(detector_ckpt.get('threshold', 0.5))
    _DETECTOR_PROBE_STEPS = int(detector_ckpt.get('probe_steps', 80))
...
def _reset(obs_arr: np.ndarray, rng: np.random.Generator) -> None:
    ...
    global _DETECTOR_TRACE, _DETECTOR_LAST_ACTION
    _LAST_RNG_ID = id(rng)
    _STEP = 0
    _MODE = None
...
def _detector_feature(obs_arr: np.ndarray, action_idx: int, step: int) -> np.ndarray:
    x = obs_arr.astype(np.float32, copy=False)
    ...
    derived = np.asarray(
        [
            ...
            front,
            right,
            float(np.sum(x[4:12:2])),
            float(np.sum(x[5:12:2])),
            float(np.sum(x[:16]) == 0.0),
            float(np.sum(x[:16])),
            float(step) / float(max(1, _DETECTOR_PROBE_STEPS - 1)),
        ],
        dtype=np.float32,
    )
...
def _scan_action(step: int, obs_arr: np.ndarray) -> str:
    cycle = int(step) % 32
    if cycle in {8, 17, 26} and float(np.sum(obs_arr[:16])) == 0.0:
        return 'FW'
    if cycle < 16:
        return 'L45'
    return 'R45'
...
@torch.no_grad()
def _detector_p_wall() -> float:
    trace = np.stack(_DETECTOR_TRACE[-_DETECTOR_PROBE_STEPS:], axis=0)
    x = torch.as_tensor(trace[None, :, :], dtype=torch.float32)
    logits = _DETECTOR(x)
...
def policy(obs: np.ndarray, rng: np.random.Generator) -> str:
    ...
    if _STEP < _DETECTOR_PROBE_STEPS:
        _DETECTOR_TRACE.append(_detector_feature(obs_arr, _DETECTOR_LAST_ACTION, _STEP))
        action = _scan_action(_STEP, obs_arr)
        _DETECTOR_LAST_ACTION = ACTION_TO_INDEX[action]
        _STEP += 1
        return action

    if _MODE is None and len(_DETECTOR_TRACE) >= _DETECTOR_PROBE_STEPS:
        p_wall = _detector_p_wall()
        _MODE = 'wall' if p_wall >= _DETECTOR_THRESHOLD else 'nowall'
        _STEP += 1
        return _WALL.act(obs_arr, rng) if _MODE == 'wall' else _NOWALL.act(obs_arr, rng)
    ...
    if _MODE == 'spin_probe':
        ...
        if _SPIN_COUNT >= 20:
            ...
            _STEP += 1
            return _WALL.act(obs_arr, rng) if _MODE == 'wall' else _NOWALL_V1.act(obs_arr, rng)
        _SPIN_COUNT += 1
        _STEP += 1
        return 'L45'
