...
class RecurrentRunner:
    def __init__(self, model: GRUPolicy, feature_config: FeatureConfig, device: torch.device, stochastic: bool):
        self.model = model
        self.device = device
        self.tracker = PoseGRUFeatureTracker(num_envs=1, config=feature_config, device=device)
    ...
    def act(self, obs: np.ndarray, rng: np.random.Generator) -> int:
        ...
        logits, _, self.hidden = self.model.forward_step(features, self.hidden, starts)
        if self.stochastic:
...
class TeacherPool:
    def __init__(self, teacher_path: str, num_envs: int, seed: int) -> None:
        teacher_mod = import_module(teacher_path, "gru_pose_teacher")
...
def collect_expert_demos(
    *,
    ...
    OBELIX = import_symbol(obelix_py, "OBELIX")
    expert_mod = import_module(expert_path, "gru_pose_expert")
    ...
    tracker = PoseGRUFeatureTracker(num_envs=1, config=feature_config, device=torch.device("cpu"))
    ...
    *,
    ...
    for epoch in range(int(epochs)):
        ...
        for start in range(0, total, batch):
            ...
            logits, _, _ = model.forward_step(x[idx], hidden0, starts)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="GRU + dead-reckoned pose PPO trainer for OBELIX")
    repo_dir = os.path.dirname(os.path.dirname(HERE))
    obelix_dir = os.path.join(repo_dir, "CS780-OBELIX")

    parser.add_argument("--obelix_py", type=str, default=os.path.join(obelix_dir, "obelix.py"))
    parser.add_argument("--obelix_torch_py", type=str, default=os.path.join(obelix_dir, "obelix_torch.py"))
    parser.add_argument("--out", type=str, default=os.path.join(HERE, "weights_best.pth"))
    ...
    parser.add_argument("--fw_bias_init", type=float, default=2.0)
    ...
    parser.add_argument("--blind_clip", type=float, default=100.0)
    parser.add_argument("--stuck_clip", type=float, default=20.0)
    parser.add_argument("--contact_clip", type=float, default=20.0)
    parser.add_argument("--same_obs_clip", type=float, default=50.0)
    parser.add_argument("--wall_hit_clip", type=float, default=20.0)
    parser.add_argument("--last_action_hist", type=int, default=5)
...
def main() -> None:
    ...
    print(
        f"[setup] feature_dim={feature_config.feature_dim} hidden_dims={encoder_dims} "
        f"gru_hidden_dim={args.gru_hidden_dim} seq_len={args.seq_len}"
    )
    ...
    try:
        while env_steps < args.total_env_steps:
            ...
            for step in range(args.rollout_steps):
                ...
                with torch.no_grad():
                    actions_t, log_probs_t, _, values_t, logits_t, next_hidden = model.act(features, hidden, starts)
            ...
            with torch.no_grad():
                ...
                _, last_values, _ = model.forward_step(next_features, hidden, starts)
            ...
            for (
                ...
                new_log_probs, entropy, values, new_logits, _ = model.evaluate_sequence(
                    feature_batch,
                    actions_batch,
                    h0_batch,
                    starts_batch,
                ...
                flat_new_log_probs = new_log_probs.reshape(-1)
