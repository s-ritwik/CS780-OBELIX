...
MOVE_ANGLES = {"L45": 45.0, "L22": 22.5, "FW": 0.0, "R22": -22.5, "R45": -45.0}
...
class ReplayedWallState:
    ...
    def sync_from_obs(self, obs: np.ndarray) -> None:
        self.stuck_flag = int(obs[17] > 0.5)
        if (not self.enable_push) and bool(obs[16] > 0.5):
    ...
    def step(self, action: str) -> None:
        self.facing_angle += float(MOVE_ANGLES[action])
        if MOVE_ANGLES[action] != 0.0:
            return
        ...
        if self.enable_push:
            ...
            if self.wall_obstacles and self.obstacles:
                ...
                if bot_side != 0.0 and box_side != 0.0 and bot_side != box_side:
                    target_x = wall_x
                    target_y = gap_y
                elif abs(self.bot_center_x - wall_x) < 60.0 and abs(self.bot_center_y - gap_y) > 25.0:
                    target_x = wall_x
                    target_y = gap_y
