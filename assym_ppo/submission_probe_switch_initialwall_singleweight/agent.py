...
class NoWallPolicy:
    def __init__(self) -> None:
        self.model: Optional[RecActorCritic] = None

    def _load_once(self) -> None:
        if self.model is not None:
            return
        bundle = torch.load(_checkpoint_path(), map_location="cpu")
        if not isinstance(bundle, dict):
            raise RuntimeError("Unsupported checkpoint format")
        raw = bundle.get("nowall")
...
class GRUWallPolicy:
    ...
    def _load_once(self) -> None:
        ...
        if not isinstance(bundle, dict):
            raise RuntimeError("Unsupported checkpoint format")
        checkpoint = bundle.get("wall_gru")
        if not isinstance(checkpoint, dict):
            raise RuntimeError("weights.pth must contain a dict entry 'wall_gru'")
...
class ProbeSwitchAgent:
    def __init__(self) -> None:
        self.wall = InitialWallPolicy()
        self.nowall = NoWallPolicy()
        self.last_episode_key: Optional[int] = None
        self.step_count = 0
    ...
    def _update_probe_stats(self, obs_arr: np.ndarray) -> None:
        ...
        self.contact_count += int(front_near >= 1 or front_count >= 4)
        self.blind_count += int(np.sum(obs_arr[:16]) == 0.0)
        self.front_total += front_count
    ...
    def act(self, obs: np.ndarray, rng: np.random.Generator) -> str:
        ...
        obs_arr = np.asarray(obs, dtype=np.float32)
        if self.step_count < PROBE_STEPS:
            self._update_probe_stats(obs_arr)
            action = self.wall.act(obs_arr, rng)
        else:
            if not self.decided:
                self.switched = (
                    (self.contact_count <= CONTACT_THRESHOLD)
                    and (self.blind_count >= BLIND_THRESHOLD)
                    and (self.front_total >= FRONT_TOTAL_THRESHOLD)
                )
                self.decided = True
            action = self.nowall.act(obs_arr, rng) if self.switched else self.wall.act(obs_arr, rng)

        self.step_count += 1
        return action


_AGENT = ProbeSwitchAgent()
