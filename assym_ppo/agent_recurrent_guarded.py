...
def _reset_episode(obs: np.ndarray) -> None:
    global _TRACKER, _HIDDEN, _PENDING_ACTION, _BLIND_STEPS, _SEARCH_DIR, _WALL_FOLLOW_STEPS, _ESCAPE_STEPS, _ESCAPE_DIR
...
def _likely_wall(c: dict[str, int]) -> bool:
    return c["active"] >= 6 or (c["front"] >= 4 and c["active"] >= 5)
...
def _wall_follow_action(c: dict[str, int]) -> str:
    global _SEARCH_DIR, _WALL_FOLLOW_STEPS
    desired_left = _SEARCH_DIR > 0
    if c["front"] >= 4:
        action = "L22" if desired_left else "R22"
    elif desired_left:
        action = "FW" if (c["left"] + c["front_left"]) > 0 else "L22"
    else:
        action = "FW" if (c["right"] + c["front_right"]) > 0 else "R22"
    _WALL_FOLLOW_STEPS = max(0, _WALL_FOLLOW_STEPS - 1)
    return action


@torch.no_grad()
def policy(obs: np.ndarray, rng: np.random.Generator) -> str:
    global _LAST_RNG_ID, _PENDING_ACTION, _HIDDEN, _BLIND_STEPS, _SEARCH_DIR, _WALL_FOLLOW_STEPS, _ESCAPE_STEPS, _ESCAPE_DIR
    _load_once()

    obs_arr = np.asarray(obs, dtype=np.float32)
    rng_id = 0 if rng is None else id(rng)
    ...
    base_action = ACTIONS[int(torch.argmax(logits, dim=1).item())]
    ...
    elif _likely_wall(c) or _WALL_FOLLOW_STEPS > 0:
        if _WALL_FOLLOW_STEPS <= 0:
            _WALL_FOLLOW_STEPS = 10
            _SEARCH_DIR *= -1
        action = _wall_follow_action(c)
    elif _BLIND_STEPS > 40:
        phase = _BLIND_STEPS % 14
        if phase < 10:
            action = "FW"
        else:
            action = "L22" if _SEARCH_DIR > 0 else "R22"
        if _BLIND_STEPS % 84 == 0:
            _SEARCH_DIR *= -1
    else:
        action = base_action
