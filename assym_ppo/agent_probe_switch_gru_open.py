...
WALL_MODULE_PATH = os.path.join(HERE, "agent_gru_wall_open.py")
...
def _load_once():
    global _WALL, _SUBMISSION, _NOWALL
    if _WALL is None:
        _WALL = _load_module(WALL_MODULE_PATH, "probe_switch_gru_wall")
    if _SUBMISSION is None:
        _SUBMISSION = _load_module(SUBMISSION_MODULE_PATH, "probe_switch_bundle")
    if _NOWALL is None:
