"""Hand-made reactive controller for OBELIX problem 1.

This policy is designed for the stationary-box setting and uses only the
18-bit observation. It keeps small per-episode memory and reacts to:

- current directional evidence from sonar/IR bits
- bit transitions relative to the previous observation
- stuck events (bit 17)

The controller is a small state machine with search / align / approach /
contact-push / recovery behavior.
"""
...
class HandmadeController:
    def __init__(self) -> None:
        ...
        self.blind_steps = 0
        self.scan_dir = 1  # +1 => left, -1 => right
        self.stuck_streak = 0
        self.escape_bias = 1
        self.escape_side = 0
        ...
        self.last_seen_side = 0  # -1 left, +1 right, 0 unknown/front
